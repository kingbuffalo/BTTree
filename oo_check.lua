...
--module("oo_check", package.seeall) --5.3 不支持 
local tbl = {}
...
function tbl.openFlag()
	checkFlag = true
end
function tbl.closeFlag()
	checkFlag = false
end

--标记类
function tbl.checkClass(clz, classNm)
	if checkFlag then
		classNameList[clz] = classNm
	end
end
--标记对象
function tbl.checkObject(clz, obj)
	if checkFlag then
...
function tbl.checkPrint(logFunc)
